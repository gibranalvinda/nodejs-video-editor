  ...
  for (const url of urls) {
    try {
      ...
      if (fs.existsSync(path.resolve('download', `${namafile}.mp4`))) {
        ...
        const backgroundImagePath = `background_${namafile}.jpg`;
        const tempOutputPath = `temp_output${namafile}.mp4`;

        // Call the video processing function and wait for it to complete
        await resizeAndOverlayVideo(inputVideoPath, outputVideoPath, backgroundImagePath, tempOutputPath);
      }
    } catch (err) {
