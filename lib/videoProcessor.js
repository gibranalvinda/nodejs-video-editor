...
const resizeAndOverlayVideo = async (
  inputVideoPath,
  outputVideoPath,
  backgroundImagePath,
  tempOutputPath,
  ...
  try {
    ...
    await ffmpeg(inputVideoPath)
      .size(`${width}x${height}`)
      .on('error', (err) => console.error('Error resizing video:', err))
      .on('end', async () => {
        ...
        await ffmpeg(inputVideoPath)
          ...
          .on('error', (err) => console.error('Error creating background:', err))
          .on('end', async () => {
            ...
            await ffmpeg()
              ...
              .on('error', (err) => console.error('Error overlaying video on background:', err))
              .on('end', async () => {
