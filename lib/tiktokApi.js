...
// Function to generate TikTok API URL with specific parameters based on the provided ID
...
// Main function for downloading TikTok content
export const TiktokDL = (url) => new Promise((resolve) => {
  // Replace "https://vm" with "https://vt" in the provided TikTok URL
  url = url.replace("https://vm", "https://vt");

  // Make a HEAD request to the modified TikTok URL to get the redirected URL
  axios.head(url)
    .then(({ request }) => {
      const { responseUrl } = request.res;
      // Extract TikTok video ID from the redirected URL
      let ID = responseUrl.match(/\d{17,21}/g);

      // If no ID is found, return an error message
      if (ID === null)
        return resolve({
          status: "error",
      ...
      // Make a GET request to the TikTok API using the generated API URL
      axios.get(_tiktokapi(ID))
        .then(({ data }) => {
          // Find the TikTok content with the matching ID
          ...
          // Extract author details from the TikTok content
          const author = {
          ...
          // Check if the TikTok content is an image or a video and structure the result accordingly
